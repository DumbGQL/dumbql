...
3. Rust specifics: test `Result::Err` paths explicitly, panics via `#[should_panic]` only when a panic is the _intended_ contract (not as a substitute for proper error handling), and `unsafe` blocks deserve extra scrutiny for UB-triggering edge cases.
