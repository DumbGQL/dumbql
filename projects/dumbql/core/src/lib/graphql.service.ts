...
export class GraphqlService {
	...
	mutate<TResponse, TVariables extends Record<string, unknown> = Record<string, unknown>>(
		...
		if (variables !== undefined && hasFiles(variables)) {
			return this.upload<TResponse>(query, variables, endpoint);
		}

		return this.withRetry(() => this.request<TResponse>(query, variables, 'mutation', endpoint));
	}
	...
	private flushBatch(): void {
		...
		this.http.post<GraphQLResponse<unknown>[]>(url, body, { headers }).pipe(
			catchError((error: unknown) => of(queue.map(() => this.toHttpError(error) as GraphQLResponse<unknown>))),
		).subscribe({
			next: (responses) => {
	...
	private upload<T>(
		...
		return this.http
			.post<GraphQLResponse<T>>(url, formData)
			.pipe(
