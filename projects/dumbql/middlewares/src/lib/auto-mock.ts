import { parse, buildSchema, type GraphQLSchema, type GraphQLType, type SelectionSetNode, type FieldNode, getNamedType, isObjectType, isListType, isScalarType, isEnumType, isNonNullType } from 'graphql';
...
function mockScalarValue(namedType: ReturnType<typeof getNamedType>, fieldName: string, ctx: MockContext): unknown {
	...
	switch (typeName) {
		case 'String': return `Sample ${fieldName}`;
		case 'Int': return Math.floor(Math.random() * 999) + 1;
		case 'Float': return parseFloat((Math.random() * 999 + 0.01).toFixed(2));
		case 'Boolean': return true;
		case 'ID': return nextId(ctx, fieldName);
...
function generateMockData(request: GraphqlRequestContext, config: AutoMockConfig): GraphQLResult<unknown> | null {
	...
	if (config.schema) {
		try { schema = getOrBuildSchema(config.schema); } catch { /* invalid, proceed without */ }
	}
	...
	try {
		...
		const data = mockObjectValue(rootType ?? { name: 'Query' } as ReturnType<typeof getNamedType>, def.selectionSet, ctx);
