...
export class DocsCache {
  ...
  protected readonly apiEntries: ApiEntry[] = [
  	// ── Core Classes ──
  	{ name: 'NormalizedCache', description: 'Low-level normalized entity store. Stores entities in a flat Map<string, CacheEntity> keyed by `TypeName:id`.', type: 'class' },
  	{ name: 'NormalizedCache.constructor', description: 'Creates a new store with optional type policies.', type: 'constructor', default: 'typePolicies?: Record<string, TypePolicy>' },
  	{ name: 'NormalizedCache.get(typename, id?)', description: 'Get entity by typename+id, or all entities of a type when id is omitted.', type: 'method' },
  	{ name: 'NormalizedCache.set(entity)', description: 'Store an entity. Overwrites if same key exists.', type: 'method' },
  	{ name: 'NormalizedCache.merge(entity)', description: 'Partial merge — only updates fields present in the input.', type: 'method' },
  	{ name: 'NormalizedCache.remove(typename, id?)', description: 'Remove entity(ies). If id is omitted, removes all of the given typename.', type: 'method' },
  	...
  	{ name: 'NormalizedCache.has(typename, id)', description: 'Returns true if an entity with the given key exists.', type: 'method' },
  	{ name: 'NormalizedCache.key(typename, id)', description: 'Builds the internal key string (`TypeName:id`).', type: 'method' },
  	{ name: 'NormalizedCache.snapshot()', description: 'Serializes all entities to a JSON string.', type: 'method' },
  	{ name: 'NormalizedCache.restore(json)', description: 'Deserializes and restores entities from a JSON string. Appends to existing data.', type: 'method' },
  	{ name: 'NormalizedCache.setTypePolicies(policies)', description: 'Updates type policies after construction.', type: 'method' },
  	{ name: 'NormalizedCache.applyOptimistic(update)', description: 'Applies an optimistic update on top of current cache.', type: 'method' },
  	{ name: 'NormalizedCache.rollbackOptimistic(id)', description: 'Removes the top-most optimistic layer by id, restoring previous state.', type: 'method' },
  	{ name: 'NormalizedCache.commitOptimistic(id)', description: 'Permanently merges the optimistic layer into the base cache, then removes it.', type: 'method' },

  	// ── CacheStore ──
  	{ name: 'CacheStore', description: 'High-level orchestrator that combines NormalizedCache, CacheGc, local state, and optional persistence.', type: 'class' },
  	{ name: 'CacheStore.constructor', description: 'Creates a CacheStore with optional config.', type: 'constructor', default: 'config?: CacheStoreConfig' },
  	{ name: 'CacheStore.cache', description: 'Public readonly reference to the underlying NormalizedCache.', type: 'property' },
  	{ name: 'CacheStore.gc', description: 'Public readonly reference to the underlying CacheGc instance.', type: 'property' },
  	{ name: 'CacheStore.write(entity)', description: 'Writes an entity to the cache with normalization. Tracks GC reference.', type: 'method' },
  	{ name: 'CacheStore.query(typename, id)', description: 'Reads a single entity. Tracks GC reference. Returns undefined if not found.', type: 'method' },
  	{ name: 'CacheStore.merge(entity)', description: 'Partial merge of an entity. Tracks GC reference.', type: 'method' },
  	{ name: 'CacheStore.evict(typename, id)', description: 'Removes an entity and releases its GC reference.', type: 'method' },
  	{ name: 'CacheStore.persist()', description: 'Writes the full cache snapshot (entities + local state) to the persistence layer.', type: 'method' },
  	{ name: 'CacheStore.serialize()', description: 'Returns the cache as a JSON string.', type: 'method' },
  	{ name: 'CacheStore.deserialize(json)', description: 'Restores cache from a JSON string. Appends to existing data.', type: 'method' },
  	{ name: 'CacheStore.collectGarbage()', description: 'Runs GC sweep and returns the number of evicted entities.', type: 'method' },
  	{ name: 'CacheStore.applyOptimistic(update)', description: 'Applies an optimistic update with key-level change tracking.', type: 'method' },
  	{ name: 'CacheStore.rollbackOptimistic(id)', description: 'Rolls back an optimistic update by id.', type: 'method' },
  	{ name: 'CacheStore.commitOptimistic(id)', description: 'Permanently commits an optimistic update.', type: 'method' },
  	{ name: 'CacheStore.readLocal(key)', description: 'Reads a local state value by key.', type: 'method' },
  	{ name: 'CacheStore.writeLocal(key, value)', description: 'Writes a local state value. Triggers watchLocal listeners.', type: 'method' },
  	{ name: 'CacheStore.watchLocal(key, listener)', description: 'Subscribes to local state changes. Returns unsubscribe function.', type: 'method' },
  	{ name: 'CacheStore.writeLocalWithTypes(key, value, types)', description: 'Writes local state scoped to a set of GraphQL type names.', type: 'method' },
  	{ name: 'CacheStore.clearLocalState()', description: 'Clears all local state values.', type: 'method' },
  	{ name: 'CacheStore.clearLocalStateByTypes(types)', description: 'Clears local state values scoped to the given type names.', type: 'method' },
  	{ name: 'CacheStore.setTypePolicies(policies)', description: 'Replaces type policies at runtime.', type: 'method' },
  	{ name: 'createCache(config?)', description: 'Factory function that returns a new CacheStore instance.', type: 'function' },

  	// ── Angular Service ──
  	{ name: 'CacheService', description: 'Angular `@Injectable()` wrapper around CacheStore. Exposes the same methods plus RxJS watchLocal().', type: 'class' },
  	{ name: 'CacheService.constructor', description: 'Accepts an optional CachePersistenceService for auto-restore on init.', type: 'constructor', default: 'persistSvc?: CachePersistenceService | null' },
  	{ name: 'CacheService.cache', description: 'Public readonly reference to the underlying NormalizedCache.', type: 'property' },
  	{ name: 'CacheService.gc', description: 'Public readonly reference to the underlying CacheGc instance.', type: 'property' },
  	{ name: 'CacheService.write(entity)', description: 'Writes an entity with normalization and GC tracking.', type: 'method' },
  	{ name: 'CacheService.query(typename, id)', description: 'Reads a single entity with GC tracking.', type: 'method' },
  	...
  	{ name: 'CacheService.watchLocal(key)', description: 'Returns an `Observable<unknown>` that emits the current value and all subsequent changes.', type: 'method' },
  	{ name: 'CacheService.writeLocal(key, value)', description: 'Writes a local state value.', type: 'method' },
  	{ name: 'CacheService.writeLocalWithTypes(key, value, types)', description: 'Writes local state scoped to a set of GraphQL type names.', type: 'method' },
  	{ name: 'CacheService.clearLocalState()', description: 'Clears all local state.', type: 'method' },
  	{ name: 'CacheService.clearLocalStateByTypes(types)', description: 'Clears local state for the given type names.', type: 'method' },
  	{ name: 'CacheService.setTypePolicies(policies)', description: 'Sets type policies at runtime.', type: 'method' },
  	{ name: 'provideCacheService(persistSvc?)', description: 'Angular provider for CacheService. Also provides GRAPHQL_CACHE token.', type: 'function' },
  	{ name: 'provideCachePersistence(config?)', description: 'Angular provider for CachePersistenceService with auto-init.', type: 'function' },

  	// ── Garbage Collector ──
  	{ name: 'CacheGc', description: 'Reference-counting garbage collector with optional TTL eviction.', type: 'class' },
  	{ name: 'CacheGc.constructor', description: 'Creates a GC instance bound to a NormalizedCache.', type: 'constructor', default: 'cache: NormalizedCache, ttlMs?: number' },
  	{ name: 'CacheGc.track(entities)', description: 'Increments reference count for each entity. Called when a query uses an entity.', type: 'method' },
  	{ name: 'CacheGc.release(entities)', description: 'Decrements reference count for each entity. Called when a query disposes.', type: 'method' },
  	{ name: 'CacheGc.sweep()', description: 'Evicts entities with zero references that have exceeded the TTL. Returns count of evicted entities.', type: 'method' },
  	{ name: 'CacheGc.refCountOf(typename, id)', description: 'Returns the current reference count for a specific entity.', type: 'method' },

  	// ── Persistence ──
  	{ name: 'CachePersistence', description: 'Zero-dependency persistence layer using localStorage with memory fallback.', type: 'class' },
  	{ name: 'CachePersistence.constructor', description: 'Creates a persistence instance.', type: 'constructor', default: 'config?: CachePersistConfig' },
  	{ name: 'CachePersistence.persist(data)', description: 'Stores data to the underlying storage.', type: 'method' },
  	{ name: 'CachePersistence.persistThrottled(data, delay?)', description: 'Throttled persist (batching writes). Default delay is 1000ms.', type: 'method' },
  	{ name: 'CachePersistence.restore()', description: 'Loads data from storage. Returns `[string, Record<string, unknown>][] | null`.', type: 'method' },
  	{ name: 'CachePersistence.clear()', description: 'Clears all data from storage.', type: 'method' },

  	// ── Interfaces ──
  	{ name: 'CacheEntity', description: 'Entity interface. Must have at minimum `__typename` and optionally `id`.', type: 'interface' },
  	{ name: 'CacheStoreConfig', description: 'CacheStore construction options.', type: 'interface' },
  	{ name: 'CacheStoreConfig.persist', description: 'Persistence config — a CachePersistence instance or plain CachePersistConfig object.', type: 'property', default: 'undefined' },
  	{ name: 'CacheStoreConfig.typePolicies', description: 'Type policies for custom cache key and merge logic.', type: 'property', default: 'undefined' },
  	{ name: 'CachePersistConfig', description: 'Persistence configuration options.', type: 'interface' },
  	{ name: 'CachePersistConfig.storageKey', description: 'Key prefix used in localStorage.', type: 'property', default: '\'dumbql_cache\'' },
  	{ name: 'CachePersistConfig.throttle', description: 'Minimum interval (ms) between consecutive persist writes.', type: 'property', default: '1000' },
  	{ name: 'CachePersistConfig.version', description: 'Schema version string. Persisted data with a different version is discarded on restore.', type: 'property', default: 'undefined' },
  	{ name: 'CachePersistConfig.maxAge', description: 'Maximum age (ms) of persisted data. Data older than this is discarded on restore.', type: 'property', default: 'undefined' },
  	{ name: 'CachePersistConfig.storage', description: 'Storage backend override: `"localStorage"` (default) or `"memory"`.', type: 'property', default: '\'localStorage\'' },
  	{ name: 'TypePolicy', description: 'Customizes caching behavior for a specific GraphQL type.', type: 'interface' },
  	{ name: 'TypePolicy.keyFields', description: 'Array of field names used to build the cache key instead of `id`. Example: `["slug"]`.', type: 'property', default: 'undefined' },
  	{ name: 'TypePolicy.merge', description: 'Custom merge strategy: `"append"`, `"prepend"`, or a function `(existing, incoming, options) => result`.', type: 'property', default: 'undefined' },
  	{ name: 'OptimisticUpdate', description: 'Describes an optimistic update with apply and rollback callbacks.', type: 'interface' },
  	{ name: 'OptimisticUpdate.id', description: 'Unique identifier for this optimistic update.', type: 'property', default: '—' },
  	{ name: 'OptimisticUpdate.apply', description: 'Function that mutates the cache to reflect the optimistic state. Signature: `(cache: Map<string, CacheEntity>) => void`.', type: 'property', default: '—' },
  	{ name: 'OptimisticUpdate.rollback', description: 'Function that restores the cache to the previous state. Signature: `(previous: Map<string, CacheEntity>) => void`.', type: 'property', default: '—' },
  ];
