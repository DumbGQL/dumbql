...
export class DocsClient {
  ...
  protected readonly apiEntries: ApiEntry[] = [
    {
      name: 'createClient(config, cache?)',
      description:
        'Factory function that creates and returns a configured DumbqlClient instance. Accepts a ClientConfig and optional CacheStore.',
      type: 'function',
    },
    {
      name: 'DumbqlClient',
      description: 'Core client class for all GraphQL operations. Methods return Promise<GraphQLResult<T>>.',
      type: 'class',
    },
    {
      name: 'DumbqlClient.constructor',
      description: 'Creates a client with config and optional cache service.',
      type: 'constructor',
      default: 'config: ClientConfig, cache?: CacheStore',
    },
    {
      name: 'DumbqlClient.query(document, variables?, endpoint?)',
      description: 'Executes a GraphQL query. Supports dedup when config.dedup is enabled.',
      type: 'method',
    },
    {
      name: 'DumbqlClient.mutate(document, variables?, endpoint?)',
      description: 'Executes a GraphQL mutation. Supports file uploads and auto-invalidates cache on success.',
      type: 'method',
    },
    {
      name: 'DumbqlClient.refetch(document, variables?, endpoint?)',
      description: 'Bypasses dedup cache and re-fetches a query.',
      type: 'method',
    },
    {
      name: 'DumbqlClient.queryStream(document, variables?, endpoint?)',
      description: 'Returns an AsyncIterable for streaming (@defer/@stream) responses via multipart/mixed.',
      type: 'method',
    },
    {
      name: 'DumbqlClient.setEndpoint(url)',
      description: 'Changes the GraphQL endpoint URL at runtime.',
      type: 'method',
    },
    {
      name: 'DumbqlClient.endpoint',
      description: 'Current GraphQL endpoint URL.',
      type: 'property',
      default: "'/graphql'",
    },
    {
      name: 'DumbqlClient.getCacheService()',
      description: 'Returns the CacheStore instance or null if not provided.',
      type: 'method',
    },
    ...
    {
      name: 'gql`...`',
      description: 'Tagged template literal. Parses a GraphQL string into DocumentNode using graphql.parse().',
      type: 'function',
    },
    ...
    { name: 'unwrap(result)', description: 'Returns data if success, null otherwise.', type: 'function' },
    {
      ...
      description: 'Returns data if success, throws Error on error status.',
      type: 'function',
    },
    {
    ...
    // ── Middleware ──
    {
      ...
      description:
        'Middleware request context: query, variables, headers, type, endpoint, extensions, method, onTypenamesExtracted.',
      type: 'interface',
    },
    {
      name: 'GraphqlMiddleware',
      description: 'Middleware function: (request, next) => Promise<GraphQLResult<unknown>>.',
      type: 'type',
    },
    { name: 'GraphqlMiddlewareNext', description: 'Next handler in the async middleware chain.', type: 'type' },
    {
      ...
      description: 'Composes middleware array into a single async pipeline using reduceRight.',
      type: 'function',
    },
    {
      name: 'authMiddleware(token, headerName?)',
      description: 'Attaches Authorization: Bearer header to requests.',
      type: 'function',
    },
    {
      name: 'loggingMiddleware(label?)',
      description: 'Logs each operation with type, query snippet, and duration.',
      type: 'function',
    },

    // ── Config Interfaces ──
    {
      name: 'DumbqlClientConfig',
      description:
        'Alias for ClientConfig. Config interface for createClient: endpoint, url, headers, errorPolicy, retryCount, middleware, retryExchange, devAuth, onError, errorHandler, subscriptions, cache, persistedQueries.',
      type: 'type',
    },
    {
      name: 'CacheConfig',
      description: 'Cache config: enabled, maxAge, serialize, typePolicies, schema.',
      type: 'interface',
    },
    {
      name: 'SubscriptionsConfig',
      description: 'WebSocket config: wsEndpoint, reconnect, reconnectInterval, lazy.',
      type: 'interface',
    },
    {
      name: 'PersistedQueriesConfig',
      description: 'Persisted queries: enabled, hash, autoPersist, useGetForHashedQueries.',
      type: 'interface',
    },
    {
    ...
    // ── Val ──
    {
      name: 'Val<T>',
      description:
        'Generic value container with built-in null handling methods. Holds a value of type T and provides nullify(), isNull(), isEmpty(), reset(), peek(), tap(), swap(), orElse(), match() and toJSON().',
      type: 'class',
    },
    ...
    {
      name: 'Val.reset()',
      description: 'Resets the value to the initial value passed to the constructor.',
      type: 'method',
    },
    { name: 'Val.peek()', description: 'Returns the value without triggering reactive tracking.', type: 'method' },
    {
      name: 'Val.tap(fn)',
      description: 'Transforms the value in-place via fn and returns this for chaining.',
      type: 'method',
    },
    { name: 'Val.swap(v)', description: 'Sets the value to v and returns the previous value.', type: 'method' },
    {
      name: 'Val.orElse(fallback)',
      description: 'Returns the value if not null, otherwise returns the fallback.',
      type: 'method',
    },
    {
      name: 'Val.match(onSome, onNone)',
      description: 'Pattern matches on null: calls onSome(value) if non-null, onNone() if null. Returns R.',
      type: 'method',
    },
