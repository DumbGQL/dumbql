...
export class DocsCodegen {
	...
	protected readonly apiEntries: ApiEntry[] = [
		{
			name: 'dumbql-codegen CLI',
			description:
				'CLI tool that scans schema and GraphQL documents, generates typed DocumentNode exports and fragment masking types.',
			type: 'cli',
		},
		{
			name: 'graphql`...` (generated)',
			description:
				'Generated template tag that returns TypedDocumentNode with fully typed query results and variables.',
			type: 'function',
		},
		{
			name: 'FragmentRef<T> (generated)',
			description: 'Generated opaque fragment reference type that enforces data masking at the type level.',
			type: 'type',
		},
		{
			name: 'useFragment(fragment, ref) (generated)',
			description: 'Generated function that unwraps a FragmentRef to access the underlying fragment data.',
			type: 'function',
		},
		{
			name: 'generateSchemaTypes(schema, config?)',
			description: 'Generates TypeScript interface/type/enum code from schema introspection data.',
			type: 'function',
		},
		{
			name: 'generateSchemaFile(schema, outputPath, config?)',
			description: 'Generates schema types and writes to a file, with optional merge mode.',
			type: 'function',
		},
		{
			name: 'SchemaData',
			description: 'Schema introspection data with types and root operation types.',
			type: 'interface',
		},
		...
		{
			name: 'SchemaType',
			description: 'A single schema type with kind, fields, input fields, enum values.',
			type: 'interface',
		},
		...
		{
			name: 'findGraphqlFiles(pattern)',
			description: 'Globs for GraphQL document files matching a pattern.',
			type: 'function',
		},
		{
			name: 'parseGraphqlFile(filePath)',
			description: 'Parses a .graphql file into a ParsedOperation.',
			type: 'function',
		},
		{
			name: 'ParsedOperation',
			description: 'Parsed operation with name, type, variables, document, and file path.',
			type: 'interface',
		},
		{ name: 'ParsedOperation.name', description: 'Operation name.', type: 'property' },
		{
			name: 'ParsedOperation.type',
		...
		{
			name: 'OperationVar',
			description: 'A single operation variable with name and GraphQL type string.',
			type: 'interface',
		},
		...
		{
			name: 'generateTypedDocumentsCode(operations, schemaTypes?, options?, fragmentTypes?)',
			description: 'Generates typed DocumentNode exports from parsed operations.',
			type: 'function',
		},
		{ name: 'DocumentCodegenOptions', description: 'Options for generating typed document code.', type: 'interface' },
		{
			name: 'DocumentCodegenOptions.prefix',
			description: 'Prefix for generated result type names.',
			type: 'property',
			default: '—',
		},
		{
			name: 'DocumentCodegenOptions.suffix',
			description: 'Suffix for generated result type names.',
			type: 'property',
			default: '—',
		},
		{
			name: 'DocumentCodegenOptions.clientPreset',
			description: 'Generate createTypedQuery() instead of gql tag.',
			type: 'property',
			default: 'false',
		},
		{
			name: 'FragmentTypeInfo',
			description: 'Metadata for a fragment type used in document generation.',
			type: 'interface',
		},
		...
		{
			name: 'FragmentTypeInfo.importRelative',
			description: 'Relative import path to the fragment file.',
			type: 'property',
		},
		{
			name: 'generateIndexCode(operations)',
			description: 'Generates a barrel index file re-exporting all operations.',
			type: 'function',
		},
		{
			name: 'mergeGeneratedTypes(existingFilePath, newContent)',
			description: 'Merges new type definitions into existing file preserving line positions.',
			type: 'function',
		},
		{
			name: 'parseFragmentFile(filePath)',
			description: 'Parses a .graphql file extracting all fragment definitions.',
			type: 'function',
		},
		{
			name: 'ParsedFragment',
			description: 'A parsed GraphQL fragment with name, type condition, and fields.',
			type: 'interface',
		},
		...
		{
			name: 'generateFragmentCode(fragments, typesCode?, clientPreset?)',
			description: 'Generates typed fragment code with FragmentRef key types.',
			type: 'function',
		},
		{
			name: 'generateFragmentIndex(fragments)',
			description: 'Generates a barrel index file re-exporting all fragments.',
			type: 'function',
		},
	];
	...
	protected readonly cliCode =
		'npx dumbql-codegen --schema ./schema.graphql --documents \'./src/**/*.graphql\' --out ./src/gql/';
