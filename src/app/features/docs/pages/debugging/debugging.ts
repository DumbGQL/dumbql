...
export class DocsDebugging {
  ...
  protected readonly apiEntries: ApiEntry[] = [
  	{ name: 'GraphqlDebugService', description: 'Debug logging service that records operation type, document, variables, timestamp, duration, result for up to 500 entries.', type: 'class' },
  	{ name: 'GraphqlDebugService.entries', description: 'Read-only array of recorded debug entries.', type: 'property' },
  	{ name: 'GraphqlDebugService.enabled', description: 'Toggle debug recording on/off.', type: 'property', default: 'true' },
  	{ name: 'GraphqlDebugService.query(document, variables?)', description: 'Executes a query and logs the operation.', type: 'method' },
  	{ name: 'GraphqlDebugService.mutate(document, variables?)', description: 'Executes a mutation and logs the operation.', type: 'method' },
  	{ name: 'GraphqlDebugService.clear()', description: 'Clears all recorded debug entries.', type: 'method' },
  	{ name: 'GraphqlDebugEntry', description: 'Recorded debug entry with timing, doc, variables, and result.', type: 'interface' },
  	...
  	{ name: 'parseFieldTree(query)', description: 'Parses a GraphQL query into a structured field tree object for visualization or analysis.', type: 'function' },
  	...
  	{ name: 'buildMutationChart(entries)', description: 'Generates a timeline chart from debug entries for visualization.', type: 'function' },
  	...
  	{ name: 'normalizeData(data, parentPath?)', description: 'Normalizes raw server data into normalized entities keyed by __typename + id.', type: 'function' },
  	...
  	{ name: 'NormalizedEntity.path', description: 'Dot-notation path to the entity in the response.', type: 'property' },
  	{ name: 'groupEntities(entries)', description: 'Groups normalized entities by their __typename.', type: 'function' },
  	{ name: 'provideDevToolsPanel()', description: 'Angular provider that registers the DevTools keyboard shortcut (Ctrl+Shift+D).', type: 'function' },
  	{ name: 'DevToolsService', description: 'Service managing DevTools panel visibility, tabs, and cache snapshots.', type: 'class' },
  	...
  	{ name: 'DevToolsPanelComponent', description: 'Standalone Angular component rendering the DevTools panel UI.', type: 'class' },
  	{ name: 'provideDumbqlDebugging(config)', description: 'Angular provider that enables detailed logging of all DumbQL operations.', type: 'function' },
  ];
