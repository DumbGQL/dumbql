...
export class DocsDownloader {
	...
	protected readonly apiEntries: ApiEntry[] = [
		{
			name: 'downloadAndStoreSchema(options)',
			description:
				'Downloads a GraphQL schema via introspection and saves schema.json and schema.graphql files. Returns paths to both files.',
			type: 'function',
		},
		{
			name: 'DownloaderOptions',
			description:
				'Configuration interface for schema download with endpoint URL, output directory, optional filename, and headers.',
			type: 'interface',
		},
		{ name: 'DownloaderOptions.endpoint', description: 'GraphQL endpoint URL to introspect.', type: 'property' },
		{
			name: 'DownloaderOptions.outputDir',
			description: 'Directory path where the schema files will be saved.',
			type: 'property',
		},
		{
			name: 'DownloaderOptions.filename',
			description: 'Custom filename for the schema JSON file. The SDL file derives its name from this.',
			type: 'property',
			default: 'schema.json',
		},
		{
			name: 'DownloaderOptions.headers',
			description: 'Additional HTTP headers for the introspection request (e.g. Authorization).',
			type: 'property',
		},
	];
