...
export class DocsErrors {
	...
	protected readonly apiEntries: ApiEntry[] = [
		{
			name: 'DumbqlError',
			description: 'Base error class for all DumbQL errors with code, timestamp, and optional context.',
			type: 'class',
		},
		...
		{
			name: 'GraphQLError',
			description: 'Error representing a GraphQL response error with locations, path, and extensions.',
			type: 'class',
		},
		...
		{
			name: 'GraphQLLocation',
			description: 'Location with line and column numbers in a GraphQL document.',
			type: 'interface',
		},
		...
		{
			name: 'NetworkError',
			description: 'Error representing a transport-level failure (timeout, offline, HTTP error).',
			type: 'class',
		},
		...
		{ name: 'NetworkError.statusText', description: 'HTTP status text if applicable.', type: 'property' },
		{
			...
			description: 'Enum of network error codes: TIMEOUT, OFFLINE, HTTP_ERROR, DNS_ERROR, ABORTED, UNKNOWN.',
			type: 'enum',
		},
		{
			name: 'CacheError',
			description: 'Error representing a cache operation failure (miss, serialization, GC, persistence).',
			type: 'class',
		},
		{
			name: 'CacheErrorCode',
			description: 'Enum of cache error codes: MISS, SERIALIZATION, GC, PERSISTENCE, INVALIDATION.',
			type: 'enum',
		},
		{
			name: 'ValidationError',
			description: 'Error representing a query or response validation failure.',
			type: 'class',
		},
		{
			name: 'ValidationErrorCode',
			description:
				'Enum of validation error codes: MISSING_VARIABLES, INVALID_QUERY, TYPE_MISMATCH, MALFORMED_RESPONSE.',
			type: 'enum',
		},
		{
			name: 'ErrorHandler',
			description: 'Configurable error handler with filter-based routing and fallback throw behavior.',
			type: 'class',
		},
		{
			name: 'ErrorHandler.on(filter, handler)',
			description: 'Registers a handler for matching error codes or filter functions. Returns this for chaining.',
			type: 'method',
		},
		{
			name: 'ErrorHandler.handle(error)',
			description: 'Processes an error through registered handlers. Returns true if handled.',
			type: 'method',
		},
		{ name: 'ErrorHandler.reset()', description: 'Clears all registered handlers.', type: 'method' },
		{
			name: 'ErrorFilter',
			description: 'Type alias for error filter function: (error: DumbqlError) => boolean.',
			type: 'type',
		},
		{
			name: 'ErrorHandlerFn',
			description: 'Type alias for error handler function returning boolean, void, or Promise.',
			type: 'type',
		},
		{ name: 'ErrorHandlerConfig', description: 'Configuration for ErrorHandler.', type: 'interface' },
		{
			name: 'ErrorHandlerConfig.throwUnhandled',
			description: 'Whether to throw errors not handled by any filter.',
			type: 'property',
			default: 'true',
		},
	];
