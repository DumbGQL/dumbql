...
export class DocsMiddlewares {
	...
	protected readonly apiEntries: ApiEntry[] = [
		{
			name: 'authRefreshMiddleware(config)',
			description:
				'Intercepts 401 responses, queues concurrent requests, calls refreshToken(), replays with new token.',
			type: 'function',
		},
		{ name: 'AuthRefreshConfig', description: 'Configuration for auth refresh middleware.', type: 'interface' },
		{
			name: 'AuthRefreshConfig.refreshToken',
			description: 'Async function that returns a new token.',
			type: 'property',
			default: '—',
		},
		{
			name: 'AuthRefreshConfig.headerName',
			description: 'Header to set the token on.',
			type: 'property',
			default: 'Authorization',
		},
		{
			name: 'AuthRefreshConfig.triggerStatuses',
			description: 'HTTP status codes that trigger a refresh.',
			type: 'property',
			default: '[401]',
		},
		{
			name: 'AuthRefreshConfig.maxAttempts',
			description: 'Max refresh attempts per request.',
			type: 'property',
			default: '1',
		},
		{
			name: 'retryExchange(config?)',
			description: 'Retries failed operations with exponential backoff and optional jitter.',
			type: 'function',
		},
		{ name: 'RetryExchangeConfig', description: 'Configuration for retry exchange middleware.', type: 'interface' },
		{
			name: 'RetryExchangeConfig.maxRetries',
			description: 'Maximum number of retry attempts.',
			type: 'property',
			default: '3',
		},
		{
			name: 'RetryExchangeConfig.initialDelay',
			description: 'Initial delay in ms before first retry.',
			type: 'property',
			default: '1000',
		},
		{
			name: 'RetryExchangeConfig.maxDelay',
			description: 'Maximum delay cap in ms.',
			type: 'property',
			default: '30000',
		},
		{
			name: 'RetryExchangeConfig.exponent',
			description: 'Backoff exponent multiplier.',
			type: 'property',
			default: '2',
		},
		{
			name: 'RetryExchangeConfig.jitter',
			description: 'Randomizes delay between 50-100% of calculated value.',
			type: 'property',
			default: 'true',
		},
		{
			name: 'RetryExchangeConfig.shouldRetry(result, attempt)',
			description: 'Custom predicate to decide whether to retry.',
			type: 'property',
			default: 'network error only',
		},
		{
			name: 'focusRefetchMiddleware(config?)',
			description: 'Refetches stale queries on visibilitychange or window.focus.',
			type: 'function',
		},
		{ name: 'FocusRefetchConfig', description: 'Configuration for focus refetch middleware.', type: 'interface' },
		{
			name: 'FocusRefetchConfig.visibilityOnly',
			description: 'Only refetch on visibilitychange, not window.focus.',
			type: 'property',
			default: 'false',
		},
		{
			name: 'FocusRefetchConfig.minStaleSeconds',
			description: 'Minimum age in seconds before a query is considered stale.',
			type: 'property',
			default: '30',
		},
		{
			name: 'offlineQueueMiddleware(config?)',
			description: 'Buffers mutations in localStorage when offline, replays on online event.',
			type: 'function',
		},
		{ name: 'OfflineQueueConfig', description: 'Configuration for offline queue middleware.', type: 'interface' },
		{
			name: 'OfflineQueueConfig.storageKey',
			description: 'localStorage key for persisted queue.',
			type: 'property',
			default: '__dumbql_offline_queue',
		},
		{
			name: 'OfflineQueueConfig.maxQueue',
			description: 'Maximum number of queued mutations.',
			type: 'property',
			default: '50',
		},
		{
			name: 'OfflineQueueConfig.autoReplay',
			description: 'Automatically replay queue on online event.',
			type: 'property',
			default: 'true',
		},
		{
			name: 'provideOfflineQueue(config?)',
			description: 'Angular provider for the offline queue service.',
			type: 'function',
		},
		...
		{
			name: 'OfflineQueueService.enqueue(query, variables)',
			description: 'Adds a mutation to the offline queue.',
			type: 'method',
		},
		{
			name: 'OfflineQueueService.replay()',
			description: 'Replays all queued mutations and returns results.',
			type: 'method',
		},
		{ name: 'OfflineQueueService.clear()', description: 'Clears all queued mutations.', type: 'method' },
		{
			name: 'OfflineQueueService.remove(id)',
			description: 'Removes a specific mutation from the queue by ID.',
			type: 'method',
		},
		{
			name: 'autoMockMiddleware(config?)',
			description: 'Generates mock GraphQL responses from a schema or field defaults.',
			type: 'function',
		},
		{ name: 'AutoMockConfig', description: 'Configuration for auto-mock middleware.', type: 'interface' },
		{
			name: 'AutoMockConfig.schema',
			description: 'Schema SDL string for type-aware mocking.',
			type: 'property',
			default: '—',
		},
		{
			name: 'AutoMockConfig.mocks',
			description: 'Custom mock resolvers keyed by type name.',
			type: 'property',
			default: '—',
		},
		{ name: 'AutoMockConfig.delay', description: 'Simulated network delay in ms.', type: 'property', default: '0' },
		{
			name: 'AutoMockConfig.passthrough',
			description: 'Fall through to real network if mock generation fails.',
			type: 'property',
			default: 'true',
		},
		{ name: 'MockResolver', description: 'Signature for custom mock resolver functions.', type: 'type' },
		{
			name: 'errorHandlerMiddleware(config)',
			description: 'Catches errors and allows custom handling with optional recoverability.',
			type: 'function',
		},
		{ name: 'ErrorHandlerConfig', description: 'Configuration for error handler middleware.', type: 'interface' },
		{
			name: 'ErrorHandlerConfig.handle(error)',
			description: 'Custom error handler returning boolean or Promise.',
			type: 'property',
		},
		{
			name: 'ErrorHandlerConfig.fallbackMessage',
			description: 'Fallback message when error has no message.',
			type: 'property',
			default: 'An error occurred',
		},
		{
			name: 'rateLimitMiddleware(config?)',
			description: 'Sliding-window rate limiter per request key.',
			type: 'function',
		},
		...
		{
			name: 'RateLimitConfig.windowMs',
			description: 'Time window in milliseconds.',
			type: 'property',
			default: '1000',
		},
		{
			name: 'RateLimitConfig.key(request)',
			description: 'Function to derive a rate-limit key from the request.',
			type: 'property',
			default: '() => "default"',
		},
		{
			name: 'dedupMiddleware()',
			description: 'Deduplicates in-flight queries sharing the same query + variables.',
			type: 'function',
		},
		{
			name: 'costEstimationMiddleware(config?)',
			description: 'Estimates query complexity and warns or blocks expensive queries.',
			type: 'function',
		},
		{ name: 'CostEstimationConfig', description: 'Configuration for cost estimation middleware.', type: 'interface' },
		{
			name: 'CostEstimationConfig.maxCost',
			description: 'Maximum allowed cost before blocking.',
			type: 'property',
			default: '1000',
		},
		{
			name: 'CostEstimationConfig.warnAt',
			description: 'Cost threshold for console warnings.',
			type: 'property',
			default: '500',
		},
		{
			name: 'CostEstimationConfig.depthFactor',
			description: 'Multiplier per nesting level.',
			type: 'property',
			default: '0.5',
		},
		{
			name: 'CostEstimationConfig.mode',
			description: 'Action when cost exceeds max: block, warn, or pass.',
			type: 'property',
			default: 'warn',
		},
		{
			name: 'estimateQueryCost(query, depthFactor?)',
			description: 'Analyzes query string and returns detailed cost breakdown.',
			type: 'function',
		},
		...
		{
			name: 'QueryCost.cost',
			description: 'Weighted cost = sum of (1 + depth × depthFactor) per field.',
			type: 'property',
		},
