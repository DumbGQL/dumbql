...
export class DocsOpentelemetry {
	...
	protected readonly apiEntries: ApiEntry[] = [
		...
		{
			name: 'MinimalTracer.constructor(config?)',
			description: 'Creates a tracer with optional exporter, sampleRate, serviceName.',
			type: 'constructor',
			default: 'TracerConfig = {}',
		},
		{
			name: 'MinimalTracer.startSpan(name, options?)',
			description: 'Creates and returns a new Span with optional parent context and attributes.',
			type: 'method',
		},
		{
			name: 'MinimalTracer.startSpanSync(name, fn, options?)',
			description: 'Creates a span, runs a synchronous function, ends span on success or error.',
			type: 'method',
		},
		{
			name: 'MinimalTracer.startSpanAsync(name, fn, options?)',
			description: 'Creates a span, runs an async function, ends span on success or error.',
			type: 'method',
		},
		{ name: 'TracerConfig', description: 'Configuration for MinimalTracer.', type: 'interface' },
		{
			name: 'TracerConfig.exporter',
			description: 'Custom SpanExporter instance.',
			type: 'property',
			default: 'consoleExporter({ pretty: true })',
		},
		{
			name: 'TracerConfig.sampleRate',
			description: 'Probability (0-1) of sampling a trace.',
			type: 'property',
			default: '1',
		},
		{
			name: 'TracerConfig.serviceName',
			description: 'Service name for identifying spans.',
			type: 'property',
			default: 'dumbql',
		},
		{
			name: 'getTracer()',
			description: 'Returns the global MinimalTracer instance, creating a default one if none set.',
			type: 'function',
		},
		{
			name: 'setTracer(tracer)',
			description: 'Sets the global tracer instance used by all OTel middleware.',
			type: 'function',
		},
		{
			name: 'SpanContext',
			description: 'W3C trace context with traceId, spanId, flags, and remote flag.',
			type: 'interface',
		},
		...
		{
			name: 'SpanContext.isRemote',
			description: 'Whether this context was propagated from a remote service.',
			type: 'property',
		},
		...
		{
			name: 'Span.addEvent(name, attributes?)',
			description: 'Records a named event with optional attributes.',
			type: 'method',
		},
		...
		{
			name: 'SpanAttributes',
			description: 'String-keyed map of attribute values (string, number, boolean, or undefined).',
			type: 'type',
		},
		...
		{
			name: 'otelMiddleware(config?)',
			description: 'Server-side middleware that captures incoming trace context from traceparent headers.',
			type: 'function',
		},
		{
			name: 'otelClientMiddleware(config?)',
			description: 'Client-side middleware that propagates trace context to outgoing requests.',
			type: 'function',
		},
		{ name: 'OtelMiddlewareConfig', description: 'Configuration for OTel middleware functions.', type: 'interface' },
		{
			name: 'OtelMiddlewareConfig.tracer',
			description: 'Custom tracer instance (defaults to global).',
			type: 'property',
			default: 'getTracer()',
		},
		{
			name: 'OtelMiddlewareConfig.attributes',
			description: 'Static attributes added to every span.',
			type: 'property',
			default: '—',
		},
		{
			name: 'OtelMiddlewareConfig.parentContext',
			description: 'Parent SpanContext for trace propagation.',
			type: 'property',
			default: '—',
		},
		{
			name: 'OtelMiddlewareConfig.maxQueryLength',
			description: 'Max query length in span attributes (0 to skip).',
			type: 'property',
			default: '500',
		},
		{
			name: 'consoleExporter(options?)',
			description: 'Built-in exporter that prints spans to console with optional pretty-print.',
			type: 'function',
		},
		{ name: 'ConsoleExporterOptions', description: 'Options for console exporter.', type: 'interface' },
		{
			name: 'ConsoleExporterOptions.pretty',
			description: 'Use console.groupCollapsed for grouped output.',
			type: 'property',
			default: 'false',
		},
		{
			name: 'parseTraceParent(header)',
			description: 'Parses a W3C traceparent header string into its component fields.',
			type: 'function',
		},
		{
			name: 'formatTraceParent(traceId, spanId, traceFlags)',
			description: 'Formats trace context into a W3C traceparent header string.',
			type: 'function',
		},
