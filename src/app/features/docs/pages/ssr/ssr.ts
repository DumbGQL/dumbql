...
export class DocsSsr {
	...
	protected readonly apiEntries: ApiEntry[] = [
		{
			name: 'SsrStreamService',
			description:
				'Injectable service for progressive SSR transfer of GraphQL data. Splits large payloads into chunks for faster TTFB.',
			type: 'class',
		},
		{
			name: 'SsrStreamService.writeChunked(key, data)',
			description: 'Serializes data to Angular TransferState under a prefixed key.',
			type: 'method',
		},
		{
			name: 'SsrStreamService.readChunked(key)',
			description: 'Reads chunked data from Angular TransferState by key.',
			type: 'method',
		},
		{ name: 'SsrStreamService.clear()', description: 'Clears all GQL-related TransferState entries.', type: 'method' },
		{
			name: 'SsrStreamConfig',
			description: 'Configuration interface for SSR stream options with custom key prefix and chunk size.',
			type: 'interface',
		},
		{
			name: 'SsrStreamConfig.key',
			description: 'Key prefix for TransferState entries.',
			type: 'property',
			default: 'gql',
		},
		{
			name: 'SsrStreamConfig.chunkSize',
			description: 'Chunk size in bytes for progressive loading.',
			type: 'property',
		},
		{
			name: 'SSR_STREAM_KEY',
			description: 'Angular InjectionToken used to provide SsrStreamConfig to the SSR stream service.',
			type: 'constant',
		},
		{
			name: 'TransferCacheService',
			description:
				'Injectable service that saves cache state on the server and restores it on the browser during SSR hydration.',
			type: 'class',
		},
		{
			name: 'TransferCacheService.save(cache)',
			description: 'Serializes and saves cache state for transfer to the browser. No-op on the browser side.',
			type: 'method',
		},
		{
			name: 'TransferCacheService.restore(cache)',
			description: 'Restores cache state from SSR transfer data. Returns true on success.',
			type: 'method',
		},
	];
