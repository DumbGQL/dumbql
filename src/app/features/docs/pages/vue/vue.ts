...
export class DocsVue {
	...
	protected readonly apiEntries: ApiEntry[] = [
		// ── Plugin ──
		{
			name: 'createDumbqlPlugin(client)',
			description: 'Vue plugin factory. Registers client globally. Usage: app.use(createDumbqlPlugin(client)).',
			type: 'function',
		},
		{
			name: 'useClient()',
			description:
				'Returns DumbqlClient instance from composition API after plugin registration. Throws if plugin not installed.',
			type: 'composable',
		},
		...
		// ── useQuery ──
		{
			...
			description:
				'Reactive query composable. All result fields are Vue refs. Supports pollInterval, skip, onCompleted, onError.',
			type: 'composable',
		},
		{
			name: 'UseQueryOptions',
			description: 'Options: { variables?, pollInterval?, skip?, onCompleted?, onError? }',
			type: 'interface',
		},
		...
		},
		{
			name: 'UseQueryResult',
			description:
				'Result (all fields are Ref<T>): { data, loading, error, errorCode, networkStatus, called, refetch, fetchMore }',
			type: 'interface',
		},
		...
		{ name: 'UseQueryResult.errorCode', description: 'Ref<ErrorCode | undefined>.', type: 'property' },
		{
			...
			description: 'Ref<NetworkStatus> — loading | ready | error | refetching | poll.',
			type: 'property',
		},
		{ name: 'UseQueryResult.called', description: 'Ref<boolean> — true once the query has fired.', type: 'property' },
		{
			...
			description: '(vars?) => Promise<GraphQLResult> — re-execute the query.',
			type: 'property',
		},
		{
			name: 'UseQueryResult.fetchMore',
			description: '(merge, vars?) => Promise<GraphQLResult> — pagination helper.',
			type: 'property',
		},
		...
		// ── useMutation ──
		{
			...
			description:
				'Returns { mutate, data, loading, error, errorCode, called } with all reactive refs. Supports optimistic updates via update option.',
			type: 'composable',
		},
		{
		...
		},
		{
			...
			description: 'Optimistic update callback: (cache) => string. Auto-committed on success, rolled back on error.',
			type: 'property',
		},
		{
			name: 'UseMutationResult',
			description: 'Result (all refs): { data, loading, error, errorCode, called, mutate }',
			type: 'interface',
		},
		...
		{
			name: 'UseMutationResult.called',
			description: 'Ref<boolean> — true once mutate has been called.',
			type: 'property',
		},
		{
			name: 'UseMutationResult.mutate',
			description: '(vars?) => Promise<GraphQLResult> — execute the mutation.',
			type: 'property',
		},
		{
			name: 'UseMutationFn',
		...
		// ── useSubscription ──
		{
			...
			description: 'Connects to a WebSocket subscription. All result fields are reactive refs.',
			type: 'composable',
		},
		{
		...
		},
		{
			...
			description: 'Callback when the subscription completes.',
			type: 'property',
		},
		{
			name: 'UseSubscriptionResult',
			description: 'Result (all refs): { data, loading, error, errorCode }',
			type: 'interface',
		},
		...
		// ── useLiveQuery ──
		{
			...
			description:
				'Combines a one-shot query with a subscription for real-time updates. All result fields are reactive refs.',
			type: 'composable',
		},
		{
		...
		{ name: 'UseLiveQueryOptions.variables', description: 'Query / subscription variables.', type: 'property' },
		{
			...
			description: 'WebSocket URL. Defaults to HTTP→WS transform.',
			type: 'property',
		},
		{
		...
		// ── useFragment ──
		{
			...
			description: 'Reads a normalized cache entity by __typename + id. Returns { data, complete }.',
			type: 'composable',
		},
		...
		{ name: 'UseFragmentResult.data', description: 'Ref<TData | null> — entity data from cache.', type: 'property' },
		{
			...
			description: 'boolean — true when the entity was found in cache.',
			type: 'property',
		},
		...
		// ── usePrefetch ──
		{
			...
			description:
				'Returns a (variables?) => Promise<GraphQLResult> prefetch function. Fires query ahead of navigation.',
			type: 'composable',
		},

		// ── QueryRef / useBackgroundQuery / useReadQuery ──
		{
			name: 'useBackgroundQuery(document, variables?)',
			description:
				'Returns QueryRef<TData> — kicks off a query and exposes reactive data/error/loading refs plus refetch.',
			type: 'composable',
		},
		{
			name: 'useReadQuery(queryRef)',
			description: 'Unwraps a QueryRef: returns { data } where data is the reactive Ref.',
			type: 'composable',
		},
		{
			name: 'QueryRef',
			description: 'Interface: { data: Ref, error: Ref, loading: Ref, refetch: () => Promise<GraphQLResult> }',
			type: 'interface',
		},
		...
		{
			name: 'RateLimitGate',
			description: 'Wrapper that shows countdown fallback when rate-limited. Slots: default, fallback.',
			type: 'component',
		},
		{
			name: 'RateLimitGateProps',
			description: 'Props: { isLimited, retryAfter?, onRetry?, error? }',
			type: 'interface',
		},
		{
		...
		// ── Directives ──
		{
			name: 'registerDirectives(app, client)',
			description:
				'Registers v-dql-mutate and v-dql-loading directives on an app instance. Called automatically by createDumbqlPlugin.',
			type: 'function',
		},
		{
			name: 'v-dql-mutate',
			description: 'Vue directive — triggers a mutation on click. Value: string (mutation) or { mutation, variables }.',
			type: 'directive',
		},
		{
			name: 'v-dql-loading',
			description: 'Vue directive — toggles .dql-loading CSS class based on boolean binding.',
			type: 'directive',
		},

		// ── Suspense composables ──
		{
			name: 'useSuspenseQuery(document, variables?)',
			description:
				'Suspense-aware query composable. Returns { data, error, loading, promise } refs. Pass promise to onServerPrefetch for SSR.',
			type: 'composable',
		},
		{
			name: 'UseSuspenseQueryResult',
			description: 'Result (all refs): { data, error, loading, promise }',
			type: 'interface',
		},
		...
		{
			name: 'UseSuspenseQueryResult.promise',
			description: 'Promise<TData | undefined> — await in onServerPrefetch for SSR hydration.',
			type: 'property',
		},
		{
			name: 'useBackgroundQuery(document, variables?)',
			description: 'Returns QueryRef<TData> — reactive query handle with data/error/loading refs and refetch.',
			type: 'composable',
		},
		...
		{
			name: 'isSuccess',
			description: 'Type guard: checks if GraphQLResult has status === "success".',
			type: 'function',
		},
		...
		// ── Val ──
		{
			...
			description:
				'Vue composable that returns a VueVal<T> — a Vue Ref<T> augmented with null-handling methods. Fully reactive.',
			type: 'composable',
		},
		{
			name: 'VueVal<T>',
			description: 'Extends Ref<T> with: nullify, isNull, isEmpty, reset, tap, swap, orElse, match.',
			type: 'interface',
		},
		...
		{
			name: 'VueVal.match(onSome, onNone)',
			description: 'Runs onSome(value) if non-null, onNone() if null. Returns R.',
			type: 'method',
		},
	];
